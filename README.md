...
In Limestone, PDAs are derived using a slot number and each slot value has a time-to-leave (`TTL`) associated with it. The `TTL` is used to validate whether the slot used to derive the PDA is too old or not. When the slot is deemed too old, Limestone will not sign the instruction &mdash; i.e., PDAs have a slot "range" (`slot > current slot - TTL`) defining when they can be used as signers.
